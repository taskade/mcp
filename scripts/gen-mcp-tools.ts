...
try {
  ...
  tools.forEach((tool) => {
    ...
    if (HUMANIZED_TASKADE_ACTIONS[tool.name]) {
      annotations.title = HUMANIZED_TASKADE_ACTIONS[tool.name];
    }

    output += `
        server.tool("${tool.name}", "${tool.description}", ${generateToolInputFromParsedTool(tool)}, ${JSON.stringify(annotations)}, async (args) => {
