...
Post `!source <item> | cost <n> | platform <x>` in **#sourcing** and it's captured.

1. **Slack ▸ Watch Public Channel Messages** — channel = `#sourcing`.
...
6. **Slack ▸ Create a Message** — channel `#sourcing`, threaded on the trigger message: "📦 Logged _{{item}}_ to Inventory and queued a listing task."
...
3. **Slack ▸ Create a Message** — channel `#sales`: "🎉 SOLD: *{{Item}}* for ${{Sale Price}} (net ${{Net Profit}}) on {{Platform}}."
